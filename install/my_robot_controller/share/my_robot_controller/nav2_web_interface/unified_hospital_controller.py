...
<head>
    ...
    <title>🏥 Système de Navigation Hospitalière - Unifié</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <div class="header-content">
                <div class="header-title">
                    ...
                    <h1>Navigation Hospitalière Unifiée</h1>
                    <span class="header-icon">🤖</span>
                </div>
                <div class="status-badges">
                    <div class="status-badge active" id="system-status">
                        <span class="status-dot"></span>
                        <span id="system-mode">SLAM Actif</span>
                    </div>
        ...
        <div class="mode-selector">
            <button class="mode-btn active" id="slam-mode-btn" onclick="switchMode('slam')">
                <span>🗺️</span>
                <span>Mode SLAM</span>
            </button>
            <button class="mode-btn" id="nav2-mode-btn" onclick="switchMode('nav2')">
                <span>🧭</span>
                <span>Mode Nav2</span>
            </button>
        </div>
        ...
        <div class="dashboard">
            ...
            <div class="panel">
                ...
                <div class="nav-stats">
                    ...
                    <div class="stat-item">
                        <div class="stat-value" id="map-coverage">0</div>
                        <div class="stat-label" id="coverage-label">Couverture</div>
                    </div>
                    <div class="stat-item">
                ...
                <div class="action-btns" id="action-buttons">
                    <!-- SLAM mode buttons -->
    ...
    <script>
        ...
        function switchMode(mode) {
            ...
            document.getElementById('slam-mode-btn').classList.remove('active');
            document.getElementById('nav2-mode-btn').classList.remove('active');
            
            if (mode === 'slam') {
                document.getElementById('slam-mode-btn').classList.add('active');
                document.getElementById('system-mode').textContent = 'SLAM Actif';
                document.getElementById('coverage-label').textContent = 'Couverture';
                
                // Update action buttons for SLAM mode
                ...
                document.getElementById('nav2-mode-btn').classList.add('active');
                document.getElementById('system-mode').textContent = 'Nav2 Actif';
                document.getElementById('coverage-label').textContent = 'Restant (m)';
                
                // Update action buttons for Nav2 mode
                document.getElementById('action-buttons').innerHTML = `
        ...
        async function goToWaypoint(index) {
            ...
            try {
                await fetch('/goto_waypoint', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({...wp, mode: currentMode})
                });
                isNavigating = true;
                navStartTime = Date.now();
                lastUpdateTime = Date.now();
        ...
        async function resetPose() {
            showNotification('Réinitialisation...', 'info');
            try {
        ...
        async function relocalize() {
            showNotification('Relocalisation...', 'info');
            try {
            ...
            try {
                ...
                if (data.stats) {
                    document.getElementById('map-coverage').textContent = 
                        currentMode === 'slam' ? data.stats.coverage + '%' : data.stats.remaining.toFixed(1);
                }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            drawMap();
            showNotification('Système Unifié initialisé', 'success');
        });
    </script>
</body>
</html>
...
class UnifiedHospitalController(Node):
    def __init__(self):
        ...
        self.get_logger().info('🏥 Unified Hospital Controller initialized')
    ...
    def navigate_to_pose_nav2(self, x, y, theta=0.0):
        ...
        self.get_logger().info(f'🎯 Nav2: Navigating to ({x:.2f}, {y:.2f}, {theta:.2f})')
    ...
    def navigate_to_pose_slam(self, x, y):
        """Navigate using simple goal pose (SLAM mode)"""
        goal = PoseStamped()
        ...
        self.get_logger().info(f'🗺️ SLAM: Navigating to ({x:.2f}, {y:.2f})')
        return True
...
def set_speed():
    data = request.json
    if 'linear' in data:
        controller.linear_speed = float(data['linear'])
    if 'angular' in data:
        controller.angular_speed = float(data['angular'])
...
def goto_waypoint():
    ...
    x = float(waypoint['x'])
    y = float(waypoint['y'])
    mode = waypoint.get('mode', 'slam')
    
    if mode == 'nav2':
        success = controller.navigate_to_pose_nav2(x, y)
    else:
        success = controller.navigate_to_pose_slam(x, y)
...
def relocalize():
    controller.set_initial_pose(
        controller.robot_pose['x'],
        controller.robot_pose['y'],
...
def run_flask():
    ...
    print("\n" + "="*80)
    print("🌐 DÉMARRAGE DU SERVEUR WEB UNIFIÉ")
    print("="*80)
...
def main(args=None):
    ...
    print("\n" + "="*80)
    print("🏥 SYSTÈME DE NAVIGATION HOSPITALIÈRE UNIFIÉ")
    print("="*80)
    ...
    print("="*80)
    print("\n✨ Fonctionnalités:")
    print("   • 🗺️  Mode SLAM - Cartographie en temps réel")
    print("   • 🧭 Mode Nav2 - Navigation intelligente")
    print("   • 🎮 Contrôle manuel (clavier + interface)")
    print("   • 🏥 8 salles hospitalières prédéfinies")
    print("   • 📍 Waypoints personnalisables")
    print("   • 📡 Visualisation LiDAR temps réel")
    print("   • 💾 Sauvegarde de carte (SLAM)")
    print("   • ⏹️  Annulation navigation (Nav2)")
    print("   • 🌐 Accès réseau depuis autre machine")
    print("="*80)
    print("\n🎮 Contrôles:")
    print("   • Interface Web: http://{local_ip}:5000")
    print("   • Clavier: W/A/S/D ou ← ↑ → ↓")
    print("   • Souris: Clic sur carte pour waypoints")
    print("   • Sélecteur: Basculer entre SLAM et Nav2")
    print("="*80)
